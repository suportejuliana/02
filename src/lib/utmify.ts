...
export const captureUtms = (): UtmParams => {
  ...
  if (Object.keys(fromUrl).length > 0) {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(fromUrl)); } catch {}
    return fromUrl;
  }
  try {
...
export const trackPurchase = (params: {
  orderId: string; value: number; email?: string; phone?: string; name?: string;
}) => {
  if (typeof window === "undefined") return;
  try {
    const w = window as any;
    if (typeof w.fbq === "function") w.fbq("track", "Purchase", { value: params.value, currency: "BRL" });
  } catch (e) { console.warn("trackPurchase error:", e); }
};
